...
- fields:
  ...
  - uri
  - contentType
  - partType   // either "base64" or "xml"
  - data
  - base64
  - xml        // string form of xml
  - xDoc       // XDocument, if xml is parsed
...
(function (root) {
    ...
    function defineOpenXml(Enumerable, Ltxml, XEnumerable) {
        ...
        function openFromBase64Internal(pkg, base64data) {
            ...
            pkg.ctXDoc = XDocument.parse(ctf.xml);
            ...
            for (var part in pkg.parts) {
                ...
                if (!openXml.util.endsWith(ct, "xml")) {
                    thisPart.base64 = openXml.util.encode64(thisPart.data);
        ...
        function openFromFlatOpcInternal(pkg, flatOpc) {
            var doc = XDocument.parse(flatOpc);
            ...
            for (var part in this.parts) {
                ...
                if (thisPart !== null) {
                    if (part === "[Content_Types].xml") {
                        ...
                        var isLtxml = false;
                        if (openXml.util.endsWith(ct, "xml")) {
                            if (thisPart.xDoc === null) {
                                partContent = thisPart.xml;
                            }
                            else {
                        ...
                        if (type !== 'xml') {
                            zip.file(name, partContent, { base64: true });
                        }
                        else {
                            if (isLtxml) {
                                zip.file(name, partContent.toString(false));
                            }
                            else {
                                zip.file(name, partContent);
                            }
                        }
                    }
                }
            }
            var b64 = zip.generate({ base64: true, compression: "deflate" });
            return b64;
        };
            ...
            for (var part in this.parts) {
                if (part !== "[Content_Types].xml") {
                    ...
                    if (thisPart !== null) {
                        ...
                        if (openXml.util.endsWith(ct, "xml")) {
                            if (thisPart.xDoc === null) {
                                partContent = XDocument.parse(openXml.util.decode_utf8(thisPart.xml)).root;
                                partContent = transformToEntities(partContent);
                            }
                            else {
                                if (thisPart.xDoc.nodeType === "Document") {
                                    partContent = thisPart.xDoc.getRoot();
                                }
                                else if (thisPart.xDoc.nodeType === "Element") {
                                    partContent = thisPart.xDoc;
                                }
                                else {
                                    throw "internal error";
        ...
        openXml.OpenXmlPart.prototype.getXDocument = function () {
            if (!this.xDoc) {
                ...
                this.xDoc = XDocument.parse(utf);
            }
            return this.xDoc;
        };

        openXml.OpenXmlPart.prototype.putXDocument = function (xDoc) {
            if (xDoc != null && xDoc !== undefined) {
                this.xDoc = xDoc;
            }
        };
